...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # Removing M2M table for field game on 'Shop'
        db.delete_table(db.shorten_name(u'igame_shop_game'))

        # Removing M2M table for field game on 'Producer'
        db.delete_table(db.shorten_name(u'igame_producer_game'))

        # Adding field 'Game.producedBy'
        db.add_column(u'igame_game', 'producedBy',
                      self.gf('django.db.models.fields.related.ForeignKey')(related_name='producedby', null=True, to=orm['igame.Producer']),
                      keep_default=False)
    ...
    def backwards(self, orm):
        # Adding M2M table for field game on 'Shop'
        m2m_table_name = db.shorten_name(u'igame_shop_game')
        db.create_table(m2m_table_name, (
            ('id', models.AutoField(verbose_name='ID', primary_key=True, auto_created=True)),
            ('shop', models.ForeignKey(orm[u'igame.shop'], null=False)),
            ('game', models.ForeignKey(orm[u'igame.game'], null=False))
        ))
        db.create_unique(m2m_table_name, ['shop_id', 'game_id'])
